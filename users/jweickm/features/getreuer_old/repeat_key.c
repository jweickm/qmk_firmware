...
static bool remember_last_key_wrapper(uint16_t keycode, keyrecord_t* record, uint8_t* remembered_mods) {
    return get_repeat_key_eligible(keycode, record) && remember_last_key_user(keycode, record, remembered_mods);
}
...
bool process_repeat_key_with_alt(uint16_t keycode, keyrecord_t* record, uint16_t repeat_keycode, uint16_t alt_repeat_keycode) {
...
#    endif // SWAP_HANDS_ENABLE
    ...
    // return KC_KP_MINUS; // No alternate key found.
